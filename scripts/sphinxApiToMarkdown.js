...
function isModuleLevelDirective(line) {
    return /^\.\. py:(class|function|data)::/.test(normLine(line));
}
function isDecorativeRstLine(line) {
    const t = normLine(line).trim();
    if (!t)
        return false;
    if (/^[=~\-_]+$/.test(t) && t.length >= 3)
        return true;
    return false;
}
/**
 ...
 */
function isRstLeafHeadingPair(lines, j) {
...
function parseRstFieldBlock(text) {
...
function formatParamsAsBullets(params) {
    if (!params.length)
        return '';
    return (params
        .map((p) => {
...
function formatMethodOrAttr(kind, signature, body) {
    const { prose, params } = parseRstFieldBlock(body);
    const sig = signature.trim();
    const kindLabel = kind === 'method' || kind === 'classmethod'
        ? kind === 'classmethod'
            ? 'classmethod'
            : 'method'
        : kind;
...
function parseIndentedDirective(lines, start, pattern) {
    for (let j = start; j < lines.length; j++) {
        const Ln = normLine(lines[j]);
        const m = Ln.match(pattern);
        if (m)
            return { match: m, indent: leadingWs(Ln), end: j };
    }
    return null;
}
function collectBodyAfterDirective(lines, dirLineIndex, dirIndent) {
    const body = [];
    ...
    while (j < lines.length) {
        ...
        if (ind <= dirIndent) {
            const isSiblingDirective = /^\s*\.\. py:(class|method|attribute|property|classmethod|function|data)::/.test(L) && ind === dirIndent;
            const isSection = /^\s*\*\*.+:\*\*/.test(L) && ind === dirIndent;
            if (isSiblingDirective || isSection)
                break;
        }
        if (ind <= dirIndent && isModuleLevelDirective(lines[j]))
            break;
        if (isRstLeafHeadingPair(lines, j))
            break;
        const raw = lines[j];
...
function formatClassBody(className, bodyLines, depth) {
    ...
    while (i < bl.length) {
        ...
        if (t.startsWith('**') && t.endsWith('**'))
            break;
        if (/^\s*\.\. py:(class|method|attribute|property|classmethod)::/.test(L))
            break;
    ...
    while (i < bl.length) {
        ...
        if (sectionM) {
            ...
            let jPeek = i;
            while (jPeek < bl.length && !bl[jPeek].trim())
                jPeek++;
            const peek = bl[jPeek] ?? '';
            ...
            if (treatAsParamTable) {
                ...
                while (i < bl.length) {
                    const K = bl[i];
                    const Kn = normLine(K);
                    if (/^\s*\*\*.+:\*\*/.test(Kn) && leadingWs(Kn) === sectionIndent)
                        break;
                    if (/^\s*\.\. py:/.test(Kn))
                        break;
                    if (isModuleLevelDirective(K))
                        break;
                    if (Kn.trim() === '') {
                        paramLines.push(K);
                        i++;
                ...
                if (params.length)
                    md += formatParamsAsBullets(params);
                if (prose)
                    md += prose + '\n\n';
            }
            continue;
        ...
        if (leadingWs(L) === 0 &&
            ...
            md += `## ${L.trim()}\n\n`;
            i += 2;
            while (i < bl.length && isDecorativeRstLine(bl[i]))
                i++;
            continue;
        }
        ...
        if (nestedClass) {
            ...
            const { text, nextLine } = collectBodyAfterDirective(bl, nestedClass.end, nestedClass.indent);
...
function formatFunctionOrData(kind, nameSig, bodyLines) {
    const md = `## ${nameSig}\n\n`;
...
function parseModuleChunkSimple(chunkLines) {
...
function sliceModuleChunkFixed(lines, start) {
    const chunk = [];
    let j = start;
    if (j >= lines.length)
        return { chunk, next: j };
    ...
    while (j < lines.length) {
        const L = lines[j];
        if (isModuleLevelDirective(normLine(L)))
            break;
        chunk.push(L);
        j++;
    }
...
function parseAllModuleChunksFixed(inner) {
    const lines = inner.split('\n').map(normLine);
    let i = 0;
    while (i < lines.length && !isModuleLevelDirective(lines[i]))
        i++;
    ...
    while (i < lines.length) {
        if (!isModuleLevelDirective(lines[i])) {
            ...
            if (t &&
                ...
                !t.includes(':')) {
                let k2 = i + 2;
                while (k2 < lines.length && isDecorativeRstLine(lines[k2]))
                    k2++;
                while (k2 < lines.length && normLine(lines[k2]) === '')
                    k2++;
                ...
                i = i + 2;
                while (i < lines.length && isDecorativeRstLine(lines[i]))
                    i++;
                continue;
            }
            i++;
 ...
 */
export function transformSphinxApiMarkdown(raw) {
    const t = raw.trimStart().replace(/^\uFEFF/, '').replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    const lines = t.split('\n').map(normLine);
    if (!lines[0]?.match(/^~{3,}text\s*$/))
        return raw;
    let end = lines.length - 1;
    while (end > 0 && lines[end] === '')
        end--;
    if (end < 1 || !lines[end].match(/^~{3,}\s*$/))
        return raw;
